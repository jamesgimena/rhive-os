...
const CustomerTrackerPage: React.FC = () => {
    const { currentProject } = useMockDB();

    // Determine current index
    const currentStageIndex = STAGES.findIndex(s => s.id === (currentProject?.current_stage || 'LEAD'));
                ...
                <div className="w-full max-w-5xl space-y-12">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-end gap-6 border-b border-gray-800 pb-8">
                        <div>
                            ...
                            <p className="text-gray-500 font-mono text-sm mt-2">{currentProject?.address}</p>
                        </div>
                        <div className="bg-gray-900/50 border border-gray-800 px-6 py-4 flex items-center gap-4" style={{ clipPath: "polygon(12px 0, 100% 0, 100% calc(100% - 12px), calc(100% - 12px) 100%, 0 100%, 0 12px)" }}>
                            ...
                            <div>
                                ...
                                <p className="text-white font-bold uppercase">{STAGES[currentStageIndex]?.name || 'Initializing'}</p>
                            </div>
                        </div>
                    </div>

                    {/* The "Pizza Tracker" UI */}
                            ...
                            return (
                                <div key={stage.id} className="relative flex flex-col items-center group">
                                    ...
                                    <div className={cn(
                                        ...
                                        isCompleted ? "bg-[#ec028b] border-[#ec028b] text-white shadow-[0_0_15px_rgba(236,2,139,0.3)]" :
                                            isActive ? "bg-black border-[#ec028b] text-[#ec028b] shadow-[0_0_20px_rgba(236,2,139,0.5)] animate-pulse" :
                        ...
                        <Card className="md:col-span-2" title="Project Update Feed">
                            <div className="space-y-6">
                                <div className="flex gap-4 border-l-2 border-[#ec028b] pl-6 py-2">
                                    <div className="shrink-0 w-10 h-10 bg-gray-900 border border-gray-800 flex items-center justify-center text-[#ec028b]" style={{ clipPath: "polygon(6px 0, 100% 0, 100% calc(100% - 6px), calc(100% - 6px) 100%, 0 100%, 0 6px)" }}>
                                        <CameraIcon className="w-5 h-5" />
                                    </div>
                                    <div>
                                        <p className="text-white font-bold text-sm">Site Inspection Photos Uploaded</p>
                                        <p className="text-gray-500 text-xs mt-1">Founders Michael and Kara have reviewed the 4k satellite data for your roof measurements.</p>
                                        <p className="text-[#ec028b] text-[10px] uppercase font-black mt-2">Today, 2:45 PM</p>
                                    </div>
                                </div>

                                <div className="flex gap-4 border-l-2 border-gray-800 pl-6 py-2 opacity-50">
                                    <div className="shrink-0 w-10 h-10 bg-gray-900 border border-gray-800 flex items-center justify-center text-gray-500" style={{ clipPath: "polygon(6px 0, 100% 0, 100% calc(100% - 6px), calc(100% - 6px) 100%, 0 100%, 0 6px)" }}>
                        ...
                        <div className="space-y-6">
                            <Card title="Quick Actions">
                                <div className="space-y-3">
                                    <button className="w-full py-3 bg-white/10 border border-white/20 text-white text-[10px] font-black uppercase tracking-widest hover:bg-white/20 transition-all" style={{ clipPath: "polygon(8px 0, 100% 0, 100% calc(100% - 8px), calc(100% - 8px) 100%, 0 100%, 0 8px)" }}>
                                        Call Michael Direct
                                    </button>
                                    <button className="w-full py-3 bg-black border border-gray-800 text-gray-400 text-[10px] font-black uppercase tracking-widest hover:text-white hover:border-gray-500 transition-all" style={{ clipPath: "polygon(8px 0, 100% 0, 100% calc(100% - 8px), calc(100% - 8px) 100%, 0 100%, 0 8px)" }}>
                                        Message Support
                                    </button>
                                </div>
                            </Card>

                            <div className="bg-[#ec028b]/5 border border-[#ec028b]/20 p-6 space-y-4" style={{ clipPath: "polygon(16px 0, 100% 0, 100% calc(100% - 16px), calc(100% - 16px) 100%, 0 100%, 0 16px)" }}>
